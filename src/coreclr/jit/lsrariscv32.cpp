...
XX                                                                           XX
XX                    Register Requirements for RISCV32                      XX
XX                                                                           XX
XX  This encapsulates all the logic for setting register requirements for    XX
XX  the RISCV32 architecture.                                                XX
XX                                                                           XX
XX                                                                           XX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
int LinearScan::BuildNode(GenTree* tree)
{
    ...
    switch (tree->OperGet())
    {
        ...
        case GT_LOCKADD:
            assert(!"-----unimplemented on RISCV32----");
            break;
        ...
        case GT_LEA:
        {
            ...
            // On RISCV32 we may need a single internal register
            // (when both conditions are true then we still only need a single internal register)
            if ((index != nullptr) && (cns != 0))
            {
                // RISCV32 does not support both Index and offset so we need an internal register
                buildInternalIntRegisterDefForNode(tree);
            }
...
{
    NYI_RISCV32("-----unimplemented on RISCV32 yet----");
    return 0;
}
#endif // FEATURE_SIMD
...
int LinearScan::BuildPutArgStk(GenTreePutArgStk* argNode)
{
    ...
    if (src->TypeIs(TYP_STRUCT))
    {
        ...
        {
            // We can use a ld/st sequence so we need two internal registers for RISCV32.
    ...
    if (blkNode->OperIsInitBlkOp())
    {
        ...
        switch (blkNode->gtBlkOpKind)
        {
            case GenTreeBlk::BlkOpKindUnroll:
            {
                ...
                {
                    // TODO-RISCV32: For larger block sizes CodeGen can choose to use 16-byte SIMD instructions.
    ...
    else
    {
        ...
        switch (blkNode->gtBlkOpKind)
        {
            case GenTreeBlk::BlkOpKindCpObjUnroll:
            {
                ...
                {
                    // TODO-RISCV32: We will use ld/st paired to reduce code size and improve performance
