...
void emitter::emitIns_S_R_R(instruction ins, emitAttr attr, regNumber reg1, regNumber tmpReg, int varx, int offs)
{
    ...
    switch (ins)
    {
        ...
        default:
            NYI_RISCV32("illegal ins within emitIns_S_R_R!");
            return;
...
void emitter::emitIns_R_S(instruction ins, emitAttr attr, regNumber reg1, int varx, int offs)
{
    ...
    switch (ins)
    {
        ...
        default:
            NYI_RISCV32("illegal ins within emitIns_R_S!");
            return;
...
{
    NYI_RISCV32("emitIns_I_I-----unimplemented/unused on RISCV32 yet----");
}
    ...
    instruction ins, emitAttr attr, regNumber reg1, regNumber reg2, insOpts opt /* = INS_OPTS_NONE */)
{
    ...
    {
        NYI_RISCV32("illegal ins within emitIns_R_R!");
    }
    ...
    instruction ins, emitAttr attr, regNumber reg1, regNumber reg2, ssize_t imm, insOpts opt /* = INS_OPTS_NONE */)
{
    ...
    if ((INS_addi <= ins && INS_srai >= ins) || (INS_addiw <= ins && INS_sraiw >= ins) ||
        (INS_lb <= ins && INS_lhu >= ins) || INS_lw == ins || INS_jalr == ins ||
        INS_flw == ins)
    {
        assert(isGeneralRegister(reg2));
        code |= (reg1 & 0x1f) << 7; // rd
    ...
    else if (INS_sw == ins || INS_sh == ins || INS_sb == ins || INS_fsw == ins)
    {
        assert(isGeneralRegister(reg2));
    ...
    else if (INS_beq <= ins && INS_bgeu >= ins)
    {
        ...
        code |= ((imm >> 5) & 0x3f) << 25;
        code |= ((imm >> 12) & 0x1) << 31;
        // TODO-RISCV32: Move jump logic to emitIns_J
    ...
    {
        NYI_RISCV32("illegal ins within emitIns_R_R_I!");
    }
    ...
    instruction ins, emitAttr attr, regNumber reg1, ssize_t imm1, ssize_t imm2, insOpts opt) /* = INS_OPTS_NONE */
{
    ...
    {
        NYI_RISCV32("illegal ins within emitIns_R_I_I!");
    }
    instrDesc* id = emitNewInstr(attr);
        ...
        (INS_feq_d <= ins && ins <= INS_fle_d) || (INS_lr_w <= ins && ins <= INS_amomaxu_d))
    {
        ...
        switch (ins)
        {
            ...
            case INS_amomaxu_d:
                break;
            default:
                NYI_RISCV32("illegal ins within emitIns_R_R_R!");
        }
...
{
    NYI_RISCV32("emitIns_R_R_R_I-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("emitIns_R_R_I_I-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("emitIns_R_R_R_R-----unimplemented/unused on RISCV32 yet----");
}
    ...
    instruction ins, emitAttr attr, regNumber reg, regNumber addrReg, CORINFO_FIELD_HANDLE fldHnd, int offs)
{
    ...
    // TODO-RISCV32: this maybe deleted.
...
{
    NYI_RISCV32("emitIns_R_AR-----unimplemented/unused on RISCV32 yet----");
}
...
{
    // TODO-RISCV32: maybe delete it on future.
    NYI_RISCV32("emitSetShortJump-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("emitIns_J_R-----unimplemented/unused on RISCV32 yet----");
}
...
void emitter::emitIns_J(instruction ins, BasicBlock* dst, int instrCount)
{
    ...
    // TODO-RISCV32: maybe deleted this.
...
void emitter::emitIns_J_cond_la(instruction ins, BasicBlock* dst, regNumber reg1, regNumber reg2)
{
    ...
    //   There is no long condition branch for RISCV32 so far.
    //   For RISCV32 , the long condition branch is like this:
...
void emitter::emitLoadImmediate(emitAttr size, regNumber reg, ssize_t imm)
{
    ...
    // TODO-RISCV32: maybe optimized via emitDataConst(), check #86790
...
unsigned emitter::emitOutputCall(const insGroup* ig, BYTE* dst, instrDesc* id, code_t code)
{
    ...
    else if (id->idIsReloc())
    {
        ...
        emitRecordRelocation(dst - 4, (BYTE*)addr, IMAGE_REL_RISCV32_PC);
    }
    else
    {
        ...
        // FIXME: need to check it carefully

        ssize_t imm = (ssize_t)(id->idAddr()->iiaAddr);
        assert((uint64_t)(imm >> 16) <= 0x7fff); // RISC-V Linux Kernel SV48
    ...
    if (!id->idIsNoGC())
    {
        // On RISCV32, as on AMD64 and LOONGARCH64, we don't change the stack pointer to push/pop args.
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 instruction
 */
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 R-Type instruction
 *
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 I-Type instruction
 *
 *  Note: Instruction types as per RISC-V Spec, Chapter 24 RV32/64G Instruction Set Listings
 *  I-Type layout:
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 S-Type instruction
 *
 *  Note: Instruction types as per RISC-V Spec, Chapter 24 RV32/64G Instruction Set Listings
 *  S-Type layout:
 *  31-------25-24---20-19--15-14------12-11-----------7-6------------0
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 U-Type instruction
 *
 *  Note: Instruction types as per RISC-V Spec, Chapter 24 RV32/64G Instruction Set Listings
 *  U-Type layout:
 *  31---------------------------------12-11-----------7-6------------0
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 B-Type instruction
 *
 *  Note: Instruction types as per RISC-V Spec, Chapter 24 RV32/64G Instruction Set Listings
 *  B-Type layout:
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 J-Type instruction
 *
 *  Note: Instruction types as per RISC-V Spec, Chapter 24 RV32/64G Instruction Set Listings
 *  J-Type layout:
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 R-Type instruction to the given buffer. Returns a
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 I-Type instruction to the given buffer. Returns a
 *  length of an encoded instruction opcode
 *
 */
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 S-Type instruction to the given buffer. Returns a
 *  length of an encoded instruction opcode
 *
 */
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 U-Type instruction to the given buffer. Returns a
 *  length of an encoded instruction opcode
 *
 */
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 B-Type instruction to the given buffer. Returns a
 *  length of an encoded instruction opcode
 *
 */
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 B-Type instruction with inverted comparation to
...
/*****************************************************************************
 *
 *  Emit a 32-bit RISCV32 J-Type instruction to the given buffer. Returns a
 *  length of an encoded instruction opcode
 *
 */
...
ssize_t emitter::emitOutputInstrJumpDistance(const BYTE* src, const insGroup* ig, instrDescJmp* jmp)
{
    ...
    assert(!jmp->idAddr()->iiaIsJitDataOffset()); // not used by riscv32 impl
...
{
    // FIXME: CHECK IT
    return static_cast<unsigned>(immediate >> 16);
}
...
BYTE* emitter::emitOutputInstr_OptsReloc(BYTE* dst, const instrDesc* id, instruction* ins)
{
    ...
    emitRecordRelocation(dstBase, id->idAddr()->iiaAddr, IMAGE_REL_RISCV32_PC);
...
void emitter::emitDispBranchOffset(const instrDesc* id, const insGroup* ig) const
{
    ...
    {
        // TODO-RISCV32-BUG: This should be a labeled offset but does not contain an iiaIGlabel
        printf("pc%+d instructions", instrCount);
        return;
    }
...
{
    printf("RISCV32 illegal instruction: 0x%08X\n", instructionCode);
}
    ...
    code_t code, const BYTE* addr, bool doffs, unsigned insOffset, const instrDesc* id, const insGroup* ig)
{
    ...
    switch (opcode)
    {
        ...
        case 0x13:
        {
            ...
            switch (opcode2)
            {
                ...
                case 0x1: // SLLI
                {
                    ...
                    imm12 &= 0x3f; // 6 BITS for SHAMT in RISCV32
                }
                break;
                case 0x2: // SLTI
                ...
                case 0x5: // SRLI & SRAI
                {
                    ...
                    imm12 &= 0x3f; // 6BITS for SHAMT in RISCV32
                }
                break;
                case 0x6: // ORI
        ...
        case 0x1b:
        {
            ...
            switch (opcode2)
            {
                ...
                case 0x1: // SLLIW
                {
                    ...
                    {
                        printf("slliw          %s, %s, %d\n", rd, rs1, imm12 & 0x1f); // 5 BITS for SHAMT in RISCV32
                    }
                    else
                    {
                ...
                case 0x5: // SRLIW & SRAIW
                {
                    ...
                    {
                        printf("srliw          %s, %s, %d\n", rd, rs1, imm12 & 0x1f); // 5BITS for SHAMT in RISCV32
                    }
        ...
        case 0x73:
        {
            ...
            {
                NYI_RISCV32("illegal ins within emitDisInsName!");
            }
            return;
        }
        case 0x53:
    ...
    instrDesc* id, bool isNew, bool doffs, bool asmfm, unsigned offset, BYTE* pCode, size_t sz, insGroup* ig)
{
    ...
    {
        // TODO-RISCV32: support different size instructions
...
{
    NYI_RISCV32("emitDispFrameRef-----unimplemented/unused on RISCV32 yet----");
}
    ...
    if (addr->isContained())
    {
        ...
        if (indir->HasIndex())
        {
            ...
            else // (offset == 0)
            {
                ...
                {
                    case EA_1BYTE:
                        assert(((ins <= INS_lhu) && (ins >= INS_lb)) || ins == INS_lwu  ||
                               ((ins <= INS_sw) && (ins >= INS_sb)));
                        if (ins <= INS_lhu || ins == INS_lwu)
                        {
                    ...
                    case EA_4BYTE:
                        assert(((ins <= INS_lhu) && (ins >= INS_lb)) || ins == INS_lwu ||
                               ((ins <= INS_sw) && (ins >= INS_sb)) || ins == INS_fsw ||
                               ins == INS_flw);
                        if (ins <= INS_lhu || ins == INS_lwu)
                        {
                            if (varTypeIsUnsigned(indir->TypeGet()))
...
{
    NYI_RISCV32("emitInsBinary-----unimplemented/unused on RISCV32 yet----");
    return REG_R0;
}
...
regNumber emitter::emitInsTernary(instruction ins, emitAttr attr, GenTree* dst, GenTree* src1, GenTree* src2)
{
    ...
    if (needCheckOv)
    {
        ...
        {
            printf("RISCV32-Invalid ins for overflow check: %s\n", codeGen->genInsName(ins));
    ...
    else
    {
        ...
        switch (dst->OperGet())
        {
            ...
            case GT_XOR:
            {
                ...
                // TODO-RISCV32-CQ: here sign-extend dst when deal with 32bit data is too conservative.
...
emitter::insExecutionCharacteristics emitter::getInsExecutionCharacteristics(instrDesc* id)
{
    ...
    // TODO-RISCV32: support this function.
