...
void CodeGen::genSaveCalleeSavedRegistersHelp(regMaskTP regsToSaveMask, int lowestCalleeSavedOffset)
{
    ...
    do
    {
        if (maskSaveRegs & 1)
        {
            emit->emitIns_R_R_I(INS_sw, EA_8BYTE, (regNumber)regNum, REG_SP, lowestCalleeSavedOffset);
    ...
    do
    {
        if (maskSaveRegs & 1)
        {
            emit->emitIns_R_R_I(INS_fsw, EA_4BYTE, (regNumber)regNum, REG_SP, lowestCalleeSavedOffset);
            compiler->unwindSaveReg((regNumber)regNum, lowestCalleeSavedOffset);
            lowestCalleeSavedOffset += REGSIZE_BYTES;
        }
        maskSaveRegs >>= 1;
...
void CodeGen::genRestoreCalleeSavedRegistersHelp(regMaskTP regsToRestoreMask, int lowestCalleeSavedOffset)
{
    ...
    do
    {
        if (maskSaveRegs < 0)
        {
            ...
            emit->emitIns_R_R_I(INS_flw, EA_8BYTE, (regNumber)regNum, REG_SP, highestCalleeSavedOffset);
    ...
    do
    {
        ...
        {
            highestCalleeSavedOffset -= REGSIZE_BYTES;
            emit->emitIns_R_R_I(INS_lw, EA_8BYTE, (regNumber)regNum, REG_SP, highestCalleeSavedOffset);
            compiler->unwindSaveReg((regNumber)regNum, highestCalleeSavedOffset);
        }
        maskSaveRegs <<= 1;
        regNum -= 1;
 ...
 *     lw s1, #(xxx-8)(sp)       ; restore callee-saved registers
 *     lw s2, #xxx(sp)
 *     lw ra, #(xxx+?+8)(sp)     ; restore RA
 *     lw fp, #(xxx+?)(sp)       ; restore FP
 *     addi sp, sp, #framesz
 *     jarl zero, ra
 */
// clang-format on
...
void CodeGen::genFuncletProlog(BasicBlock* block)
{
    ...
    // TODO-RISCV32: Implement varargs (NYI_RISCV32)
    ...
    if ((FP_offset + (genCountBits(maskSaveRegs) << 3)) <= (2040 - 16)) // no FP/RA.
    {
        ...
        GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_FP, REG_SPBASE, FP_offset);
        compiler->unwindSaveReg(REG_FP, FP_offset);

        GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_RA, REG_SPBASE, FP_offset + 8);
    ...
    if (compiler->lvaPSPSym != BAD_VAR_NUM)
    {
        if (isFilter)
        {
            ...
            // function)
            genInstrWithConstant(INS_lw, EA_PTRSIZE, REG_A1, REG_A1, genFuncletInfo.fiCallerSP_to_PSP_slot_delta,
                                 REG_A2, false);
            regSet.verifyRegUsed(REG_A1);

            // Store the PSP value (aka CallerSP)
            genInstrWithConstant(INS_sw, EA_PTRSIZE, REG_A1, REG_SPBASE, genFuncletInfo.fiSP_to_PSP_slot_delta, REG_A2,
                                 false);
        ...
        else // This is a non-filter funclet
        {
            ...
            genInstrWithConstant(INS_sw, EA_PTRSIZE, REG_A3, REG_SPBASE, genFuncletInfo.fiSP_to_PSP_slot_delta, REG_A2,
                                 false);
        }
    }
}
...
void CodeGen::genFuncletEpilog()
{
    ...
    GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_RA, REG_SPBASE, FP_offset + 8);
    compiler->unwindSaveReg(REG_RA, FP_offset + 8);

    GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_FP, REG_SPBASE, FP_offset);
    compiler->unwindSaveReg(REG_FP, FP_offset);
    ...
    if (jmpEpilog)
    {
       ...
       // If jmpNode is a fast tail call, gtNext need not be null since it could have embedded stmts.
...
#endif // FEATURE_FASTTAILCALL
        {
            ...
            {
                case IAT_VALUE:
                // TODO-RISCV32-CQ: using B/BL for optimization.
                case IAT_PVALUE:
                    ...
                    {
                        GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, indCallReg, indCallReg, 0);
...
void CodeGen::genSetPSPSym(regNumber initReg, bool* pInitRegZeroed)
{
    ...
    *pInitRegZeroed  = false;
    ...
    GetEmitter()->emitIns_S_R(INS_sw, EA_PTRSIZE, regTmp, compiler->lvaPSPSym, 0);
}
...
void CodeGen::genZeroInitFrameUsingBlockInit(int untrLclHi, int untrLclLo, regNumber initReg, bool* pInitRegZeroed)
{
    ...
    *pInitRegZeroed = false;
    ...
    if (useLoop)
    {
        ...
        // TODO-RISCV32: maybe optimize further
        GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, rAddr, 8 + padding);
        GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, rAddr, 0 + padding);
    ...
    if (uCntBytes >= REGSIZE_BYTES) // check and zero the last register-sized stack slot (odd number)
    {
        ...
        {
            GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, rAddr, padding);
        }
        else
        {
            GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, rAddr, padding);
...
BasicBlock* CodeGen::genCallFinally(BasicBlock* block)
{
    ...
    if (compiler->lvaPSPSym != BAD_VAR_NUM)
    {
        GetEmitter()->emitIns_R_S(INS_lw, EA_PTRSIZE, REG_A0, compiler->lvaPSPSym, 0);
    }
    else
    {
    ...
    else
    {
        ...
        {
            // Fall-through.
            // TODO-RISCV32-CQ: Can we get rid of this instruction, and just have the call return directly
...
void CodeGen::genSetRegToConst(regNumber targetReg, var_types targetType, GenTree* tree)
{
    switch (tree->gtOper)
    {
        case GT_CNS_INT:
        {
            ...
            // TODO-RISCV32-CQ: Currently we cannot do this for all handles because of
        ...
        case GT_CNS_DBL:
        {
            ...
            emit->emitIns_R_C(INS_flw, size, targetReg, REG_NA, hnd, 0);
        }
        break;
...
void CodeGen::genCodeForStoreLclVar(GenTreeLclVar* lclNode)
{
    ...
    if (lclNode->IsMultiReg())
    {
        ...
        NYI_RISCV32("genCodeForStoreLclVar-----unimplemented on RISCV32 yet----");
    }
    else
    {
        ...
        if (data->isContained())
        {
            ...
            // TODO-RISCV32-CQ: supporting the SIMD.
...
void CodeGen::genLclHeap(GenTree* tree)
{
    ...
    BasicBlock*          endLabel                 = nullptr; // can optimize for riscv32.
    ...
    if (size->IsCnsIntOrI())
    {
        ...
        if (compiler->info.compInitMem)
        {
            if (stpCount <= 4)
            {
                ...
                {
                    imm -= 8;
                    emit->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, REG_SPBASE, imm);
                    imm -= 8;
                    emit->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, REG_SPBASE, imm);
                    stpCount -= 1;
                }
    ...
    if (compiler->info.compInitMem)
    {
        ...
        emit->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, REG_SPBASE, 8);
        emit->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, REG_SPBASE, 0);
...
{
    NYI_RISCV32("genCodeForBswap-----unimplemented on RISCV32 yet----");
}
    ...
    else // an integer divide operation
    {
        ...
        if (tree->OperIs(GT_DIV, GT_MOD))
        {
            if ((exceptions & ExceptionSetFlags::ArithmeticException) != ExceptionSetFlags::None)
            {
                ...
                BasicBlock* sdivLabel = genCreateTempLabel(); // can optimize for riscv32.
...
void CodeGen::genCodeForInitBlkUnroll(GenTreeBlk* node)
{
    ...
    for (unsigned regSize = 2 * REGSIZE_BYTES; size >= regSize; size -= regSize, dstOffset += regSize)
    {
        ...
        {
            emit->emitIns_S_R(INS_sw, EA_8BYTE, srcReg, dstLclNum, dstOffset);
            emit->emitIns_S_R(INS_sw, EA_8BYTE, srcReg, dstLclNum, dstOffset + 8);
        }
        else
        {
            emit->emitIns_R_R_I(INS_sw, EA_8BYTE, srcReg, dstAddrBaseReg, dstOffset);
            emit->emitIns_R_R_I(INS_sw, EA_8BYTE, srcReg, dstAddrBaseReg, dstOffset + 8);
        }
    }
...
void CodeGen::genCodeForCpObj(GenTreeBlk* cpObjNode)
{
    ...
    if (dstOnStack)
    {
        ...
        while (i < slots - 1)
        {
            ...
            emit->emitIns_R_R_I(INS_lw, attr0, tmpReg, REG_WRITE_BARRIER_SRC_BYREF, 0);
            emit->emitIns_R_R_I(INS_lw, attr1, tmpReg2, REG_WRITE_BARRIER_SRC_BYREF, TARGET_POINTER_SIZE);
            emit->emitIns_R_R_I(INS_addi, attrSrcAddr, REG_WRITE_BARRIER_SRC_BYREF, REG_WRITE_BARRIER_SRC_BYREF,
                                2 * TARGET_POINTER_SIZE);
            emit->emitIns_R_R_I(INS_sw, attr0, tmpReg, REG_WRITE_BARRIER_DST_BYREF, 0);
            emit->emitIns_R_R_I(INS_sw, attr1, tmpReg2, REG_WRITE_BARRIER_DST_BYREF, TARGET_POINTER_SIZE);
        ...
        while (i < slots)
        {
            if (!layout->IsGCPtr(i))
            {
                ...
                if ((i + 1 < slots) && !layout->IsGCPtr(i + 1))
                {
                    ...
                    emit->emitIns_R_R_I(INS_lw, EA_8BYTE, tmpReg, REG_WRITE_BARRIER_SRC_BYREF, 0);
                    emit->emitIns_R_R_I(INS_lw, EA_8BYTE, tmpReg2, REG_WRITE_BARRIER_SRC_BYREF, TARGET_POINTER_SIZE);
                    emit->emitIns_R_R_I(INS_addi, attrSrcAddr, REG_WRITE_BARRIER_SRC_BYREF, REG_WRITE_BARRIER_SRC_BYREF,
                                        2 * TARGET_POINTER_SIZE);
                    emit->emitIns_R_R_I(INS_sw, EA_8BYTE, tmpReg, REG_WRITE_BARRIER_DST_BYREF, 0);
                    emit->emitIns_R_R_I(INS_sw, EA_8BYTE, tmpReg2, REG_WRITE_BARRIER_DST_BYREF, TARGET_POINTER_SIZE);
                    emit->emitIns_R_R_I(INS_addi, attrDstAddr, REG_WRITE_BARRIER_DST_BYREF, REG_WRITE_BARRIER_DST_BYREF,
                                        2 * TARGET_POINTER_SIZE);
    ...
    if (cpObjNode->IsVolatile())
    {
        ...
        // TODO-RISCV32: there is only BARRIER_FULL for RISCV32.
...
instruction CodeGen::genGetInsForOper(GenTree* treeNode)
{
    ...
    else
    {
        switch (oper)
        {
            ...
            case GT_MUL:
                // TODO-RISCV32-CQ: Need to implement for complex cases
                if ((attr == EA_8BYTE) || (attr == EA_BYREF))
                {
            ...
            case GT_AND_NOT:
                NYI_RISCV32("GT_AND_NOT-----unimplemented/unused on RISCV32 yet----");
                break;
            ...
            case GT_ROR:
                NYI_RISCV32("GT_ROR-----unimplemented/unused on RISCV32 yet----");
                break;
...
void CodeGen::genCodeForReturnTrap(GenTreeOp* tree)
{
    ...
    if (addr == nullptr)
    {
        ...
        else
        {
            ...
            GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, callTarget, callTarget, 0);
        }
    ...
    // TODO-RISCV32: can optimize further !!!
...
void CodeGen::genLongToIntCast(GenTree* cast)
{
    ...
    if (cast->gtOverflow())
    {
#pragma message("That's terribly bad that it is not ported!")
#if 0
        //
...
void CodeGen::genCodeForSwap(GenTreeOp*)
{
    ...
    NYI_RISCV32("genCodeForSwap-----unimplemented/unused on RISCV32 yet----");
}
...
static void emitLoadConstAtAddr(emitter* emit, regNumber dstRegister, ssize_t imm)
{
    ...
    emit->emitIns_R_R_I(INS_lw, EA_PTRSIZE, dstRegister, dstRegister, (low & 0x3ff));
}
...
void CodeGen::genEmitHelperCall(unsigned helper, int argSize, emitAttr retSize, regNumber callTargetReg /*= REG_NA */)
{
    ...
    if (addr == nullptr)
    {
        ...
        if (compiler->opts.compReloc)
        {
            // TODO-RISCV32: here the jal is special flag rather than a real instruction.
            GetEmitter()->emitIns_R_AI(INS_jal, EA_PTR_DSP_RELOC, callTarget, (ssize_t)pAddr);
        }
        else
        {
...
{
    NYI_RISCV32("genSIMDIntrinsic-----unimplemented/unused on RISCV32 yet----");
}

insOpts CodeGen::genGetSimdInsOpt(emitAttr size, var_types elementType)
{
    NYI_RISCV32("genGetSimdInsOpt-----unimplemented/unused on RISCV32 yet----");
...
{
    NYI_RISCV32("getOpForSIMDIntrinsic-----unimplemented/unused on RISCV32 yet----");
    return INS_invalid;
}
...
{
    NYI_RISCV32("genSIMDIntrinsicInit-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicInitN-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicUnOp-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicWiden-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicNarrow-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicBinOp-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicRelOp-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicDotProduct-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicGetItem-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicSetItem-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicUpperSave-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genSIMDIntrinsicUpperRestore-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genStoreIndTypeSIMD12-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genLoadIndTypeSIMD12-----unimplemented/unused on RISCV32 yet----");
}
...
{
    NYI_RISCV32("genStoreLclTypeSIMD12-----unimplemented/unused on RISCV32 yet----");
}
...
// the runtime side requires the codegen here to be consistent
        ...
        case GT_LABEL:
            ...
            emit->emitIns_R_L(INS_lw, EA_PTRSIZE, genPendingCallLabel, targetReg);
            break;
        ...
        default:
        {
            ...
            NYIRAW(message);
#else
            NYI_RISCV32("some node type in genCodeForTreeNode-----unimplemented/unused on RISCV32 yet----");
#endif
        }
        break;
    }
...
void CodeGen::genSetGSSecurityCookie(regNumber initReg, bool* pInitRegZeroed)
{
    ...
    if (compiler->gsGlobalSecurityCookieAddr == nullptr)
    {
        ...
        emit->emitIns_S_R(INS_sw, EA_PTRSIZE, initReg, compiler->lvaGSSecurityCookie, 0);
    }
    else
    {
        if (compiler->opts.compReloc)
        ...
        else
        {
            ...
            emit->emitIns_R_R_I(INS_lw, EA_PTRSIZE, initReg, initReg, 0);
        }
...
void CodeGen::genEmitGSCookieCheck(bool pushReg)
{
    ...
    else
    {
        ...
        // GetEmitter()->emitIns_R_R_I(INS_lw_d, EA_PTRSIZE, regGSConst, regGSConst, 0);
        if (compiler->opts.compReloc)
        {
        ...
        {
            // TODO-RISCV32: maybe optimize further!
            UINT32 high = ((ssize_t)compiler->gsGlobalSecurityCookieAddr) >> 16;
            if (((high + 0x800) >> 12) != 0)
            {
            ...
            UINT32 low = ((ssize_t)compiler->gsGlobalSecurityCookieAddr) & 0xffff;
            ...
            GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, regGSConst, regGSConst, low & 0x3FF);
        }
        regSet.verifyRegUsed(regGSConst);
    }
    // Load this method's GS value from the stack frame
    GetEmitter()->emitIns_R_S(INS_lw, EA_PTRSIZE, regGSValue, compiler->lvaGSSecurityCookie, 0);
...
{
    NYI_RISCV32("genIntrinsic-----unimplemented/unused on RISCV32 yet----");
}
...
void CodeGen::genPutArgStk(GenTreePutArgStk* treeNode)
{
    ...
    if (!source->TypeIs(TYP_STRUCT)) // a normal non-Struct argument
    {
        ...
        {
            NYI_RISCV32("SIMD in genPutArgStk-----unimplemented/unused on RISCV32 yet----");
        }
...
void CodeGen::genCodeForShift(GenTree* tree)
{
    ...
    {
        regNumber tempReg  = internalRegisters.GetSingle(tree);
        unsigned  immWidth = emitter::getBitWidth(size); // For RISCV32, immWidth will be set to 32 or 64
    ...
    else
    {
        ...
        else
        {
            ...
            // should check shiftByImm for riscv32-ins.
            unsigned immWidth = emitter::getBitWidth(size); // For RISCV32, immWidth will be set to 32 or 64
...
void CodeGen::genCodeForCpBlkUnroll(GenTreeBlk* cpBlkNode)
{
    ...
    if (size >= 2 * REGSIZE_BYTES)
    {
             ...
             size -= regSize, srcOffset += regSize, dstOffset += regSize)
        {
            if (srcLclNum != BAD_VAR_NUM)
            {
                emit->emitIns_R_S(INS_lw, EA_8BYTE, tempReg, srcLclNum, srcOffset);
                emit->emitIns_R_S(INS_lw, EA_8BYTE, tempReg2, srcLclNum, srcOffset + 8);
            }
            else
            {
                emit->emitIns_R_R_I(INS_lw, EA_8BYTE, tempReg, srcAddrBaseReg, srcOffset);
                emit->emitIns_R_R_I(INS_lw, EA_8BYTE, tempReg2, srcAddrBaseReg, srcOffset + 8);
            }

            if (dstLclNum != BAD_VAR_NUM)
            {
                emit->emitIns_S_R(INS_sw, EA_8BYTE, tempReg, dstLclNum, dstOffset);
                emit->emitIns_S_R(INS_sw, EA_8BYTE, tempReg2, dstLclNum, dstOffset + 8);
            }
            else
            {
                emit->emitIns_R_R_I(INS_sw, EA_8BYTE, tempReg, dstAddrBaseReg, dstOffset);
                emit->emitIns_R_R_I(INS_sw, EA_8BYTE, tempReg2, dstAddrBaseReg, dstOffset + 8);
            }
        }
    }
...
void CodeGen::genCodeForInitBlkLoop(GenTreeBlk* initBlkNode)
{
    ...
    GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, dstReg, 0);
    if (size > TARGET_POINTER_SIZE)
    {
        ...
        // *tempReg = 0
        GetEmitter()->emitIns_R_R_I(INS_sw, EA_PTRSIZE, REG_R0, tempReg, 0);
        // tempReg = tempReg - 8
...
{
    NYI_RISCV32("Varargs not supported");
}
                                      ...
                                      unsigned epilogSize DEBUGARG(void* codePtr))
{
    ...
    {
        NYI_RISCV32("compDbgEnc in genCreateAndStoreGCInfo-----unimplemented/unused on RISCV32 yet----");
    }
...
void CodeGen::genLeaInstruction(GenTreeAddrMode* lea)
{
    ...
    //
    // TODO-RISCV32-CQ: The purpose of the GT_LEA node is to directly reflect a single target architecture
    ...
    //             produce LEAs that are a 1:1 relationship to the RISCV32 architecture.
    ...
    else if (lea->HasIndex())
    {
        ...
        // This is currently disabled in RISCV32 since we need to adjust lower to account
        // for the simpler instructions RISCV32 supports.
        // TODO-RISCV32-CQ:  Fix this and let LEA optimize arithmetic trees too.
        assert(!"We shouldn't see a baseless address computation during CodeGen for RISCV32");
    }
    ...
    else
    {
        ...
        if (addr == nullptr)
        {
            ...
            else
            {
                ssize_t imm = 9 << 2;
                emit->emitIns_R_R_I(ins, EA_PTRSIZE, reg1, reg2, imm);
                // TODO-RISCV32-CQ: In the future we may consider using emitter::emitLoadImmediate instead,
...
void CodeGen::instGen_MemoryBarrier(BarrierKind barrierKind)
{
    ...
    // TODO-RISCV32: Use the exact barrier type depending on the CPU.
 ...
 *      fsw f18, #(offset+8)(sp)
 *      ; ...
 *      fsw f27, #(offset+8*11)(sp)
 *
 *      ; save int regs
 *      sw s1, #offset2(sp)
 *      sw s2, #(offset2+4)(sp)
 *      ; ...
 *      sw s11, #(offset+4*10)(sp)
 *
 *      ; save ra, fp
 *      sw ra, #offset3+4(sp)     ; save RA (8 bytes)
 *      sw fp, #(offset3)(sp)     ; save FP (8 bytes)
 *
 * Notes:
...
void CodeGen::genPushCalleeSavedRegisters(regNumber initReg, bool* pInitRegZeroed)
{
    ...
    if (compiler->info.compIsVarArgs)
    {
        ...
        NYI_RISCV32("genPushCalleeSavedRegisters unsupports compIsVarArgs");
    }
...
void CodeGen::genPopCalleeSavedRegisters(bool jmpEpilog)
{
    ...
    emit->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_RA, REG_SPBASE, FP_offset + 8);
    compiler->unwindSaveReg(REG_RA, FP_offset + 8);

    emit->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_FP, REG_SPBASE, FP_offset);
    compiler->unwindSaveReg(REG_FP, FP_offset);
...
void CodeGen::genProfilingEnterCallback(regNumber initReg, bool* pInitRegZeroed)
{
    ...
    if (compiler->compProfilerMethHndIndirected)
    {
        ...
        GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_PROFILER_ENTER_ARG_FUNC_ID, REG_PROFILER_ENTER_ARG_FUNC_ID,
                                    0);
    }
    else
    {
...
void CodeGen::genProfilingLeaveCallback(unsigned helper /*= CORINFO_HELP_PROF_FCN_LEAVE*/)
{
    ...
    if (compiler->compProfilerMethHndIndirected)
    {
        ...
        GetEmitter()->emitIns_R_R_I(INS_lw, EA_PTRSIZE, REG_PROFILER_LEAVE_ARG_FUNC_ID, REG_PROFILER_LEAVE_ARG_FUNC_ID,
                                    0);
    }
    else
    {
