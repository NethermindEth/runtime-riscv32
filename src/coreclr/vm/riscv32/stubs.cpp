...
void UpdateRegDisplayFromCalleeSavedRegisters(REGDISPLAY * pRD, CalleeSavedRegisters * pCalleeSaved)
{
    ...
    pContextPointers->S1 = (PDWORD)&pCalleeSaved->s1;
    pContextPointers->S2 = (PDWORD)&pCalleeSaved->s2;
    pContextPointers->S3 = (PDWORD)&pCalleeSaved->s3;
    pContextPointers->S4 = (PDWORD)&pCalleeSaved->s4;
    pContextPointers->S5 = (PDWORD)&pCalleeSaved->s5;
    pContextPointers->S6 = (PDWORD)&pCalleeSaved->s6;
    pContextPointers->S7 = (PDWORD)&pCalleeSaved->s7;
    pContextPointers->S8 = (PDWORD)&pCalleeSaved->s8;
    pContextPointers->S9 = (PDWORD)&pCalleeSaved->s9;
    pContextPointers->S10 = (PDWORD)&pCalleeSaved->s10;
    pContextPointers->S11 = (PDWORD)&pCalleeSaved->s11;
    pContextPointers->Gp = (PDWORD)&pCalleeSaved->gp;
    pContextPointers->Tp = (PDWORD)&pCalleeSaved->tp;
    pContextPointers->Fp = (PDWORD)&pCalleeSaved->fp;
    pContextPointers->Ra  = (PDWORD)&pCalleeSaved->ra;
}
...
void FaultingExceptionFrame::UpdateRegDisplay_Impl(const PREGDISPLAY pRD, bool updateFloats)
{
    ...
    pRD->pCurrentContextPointers->S1 = (PDWORD)&m_ctx.S1;
    pRD->pCurrentContextPointers->S2 = (PDWORD)&m_ctx.S2;
    pRD->pCurrentContextPointers->S3 = (PDWORD)&m_ctx.S3;
    pRD->pCurrentContextPointers->S4 = (PDWORD)&m_ctx.S4;
    pRD->pCurrentContextPointers->S5 = (PDWORD)&m_ctx.S5;
    pRD->pCurrentContextPointers->S6 = (PDWORD)&m_ctx.S6;
    pRD->pCurrentContextPointers->S7 = (PDWORD)&m_ctx.S7;
    pRD->pCurrentContextPointers->S8 = (PDWORD)&m_ctx.S8;
    pRD->pCurrentContextPointers->S9 = (PDWORD)&m_ctx.S9;
    pRD->pCurrentContextPointers->S10 = (PDWORD)&m_ctx.S10;
    pRD->pCurrentContextPointers->S11 = (PDWORD)&m_ctx.S11;
    pRD->pCurrentContextPointers->Fp = (PDWORD)&m_ctx.Fp;
    pRD->pCurrentContextPointers->Gp = (PDWORD)&m_ctx.Gp;
    pRD->pCurrentContextPointers->Tp = (PDWORD)&m_ctx.Tp;
    pRD->pCurrentContextPointers->Ra = (PDWORD)&m_ctx.Ra;
...
{
    _ASSERTE(!"RISCV32: not implementation on riscv64!!!");
}
...
void StubLinkerCPU::EmitMovConstant(IntReg reg, UINT32 imm)
{
...
{
    Emit32(ITypeInstr(0x3, 0x2, dest, srcAddr, offset));  // lw
    LOG((LF_STUBS, LL_EVERYTHING, "lw %s, %i(%s)\n", intRegAbiNames[dest], offset, intRegAbiNames[srcAddr]));
}

void StubLinkerCPU:: EmitStore(IntReg src, IntReg destAddr, int offset)
{
    Emit32(STypeInstr(0x23, 0x2, destAddr, src, offset));  // sw
    LOG((LF_STUBS, LL_EVERYTHING, "sw %s, %i(%s)\n", intRegAbiNames[src], offset, intRegAbiNames[destAddr]));
}
...
VOID StubLinkerCPU::EmitComputedInstantiatingMethodStub(MethodDesc* pSharedMD, struct ShuffleEntry *pShuffleEntryArray, void* extraArg)
{
    ...
    if (argit.HasParamType())
    {
        ...
        regHidden += argRegBase;//NOTE: RISCV32 should start at a0=10;
...
PCODE DynamicHelpers::CreateHelper(LoaderAllocator * pAllocator, TADDR arg, PCODE target)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA0, RegT0, 8);// lw a0, 8(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 12);// lw t0, 12(t0)
    p += 4;
...
void DynamicHelpers::EmitHelperWithArg(BYTE*& p, size_t rxOffset, LoaderAllocator * pAllocator, TADDR arg, PCODE target)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA1, RegT0, 8);// lw a1, 8(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 12);// lw t0, 12(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x67, 0, RegR0, RegT0, 0);// jalr zero, 0(t0)
...
PCODE DynamicHelpers::CreateHelper(LoaderAllocator * pAllocator, TADDR arg, TADDR arg2, PCODE target)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA0, RegT0, 12);// lw a0, 12(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA1, RegT0, 16);// lw a1, 16(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 20);// lw t0, 20(t0)
    p += 4;
...
PCODE DynamicHelpers::CreateHelperArgMove(LoaderAllocator * pAllocator, TADDR arg, PCODE target)
{
    ...
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA0, RegT0, 12);// lw a0, 12(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 16);// lw t0, 16(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x67, 0, RegR0, RegT0, 0);// jalr x0, 0(t0)
    p += 4;
...
PCODE DynamicHelpers::CreateReturnConst(LoaderAllocator * pAllocator, TADDR arg)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA0, RegT0, 8);// lw a0, 16(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x67, 0, RegR0, RegRa, 0);// ret
    p += 4;
    *(DWORD*)p = ITypeInstr(0x13, 0, RegR0, RegR0, 0);// nop, padding to make 8 byte aligned
...
PCODE DynamicHelpers::CreateHelperWithTwoArgs(LoaderAllocator * pAllocator, TADDR arg, PCODE target)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA2, RegT0, 8);// lw a2,8(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 12);// ld t0,12(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x67, 0, RegR0, RegT0, 0);// jalr x0, 0(t0)
    p += 4;
...
PCODE DynamicHelpers::CreateHelperWithTwoArgs(LoaderAllocator * pAllocator, TADDR arg, TADDR arg2, PCODE target)
{
    ...
    *(DWORD*)p = UTypeInstr(0x17, RegT0, 0);// auipc t0, 0
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA2, RegT0, 24);// lw a2,12(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA3, RegT0, 32);// lw a3,16(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT0, RegT0, 40);// lw t0,20(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x67, 0, RegR0, RegT0, 0);// jalr x0, 0(t0)
    p += 4;
    *(DWORD*)p = ITypeInstr(0x13, 0, RegR0, RegR0, 0);// nop, padding to make 8 byte aligned
    ...
    else
    {
        ...
        for (WORD i = 0; i < pLookup->indirections; i++)
        {
            if (i == pLookup->indirections - 1 && pLookup->sizeOffset != CORINFO_NO_SIZE_CHECK)
            {
                ...
                if (pLookup->sizeOffset > 2047)
                {
                    ...
                    *(DWORD*)p = RTypeInstr(0x33, 0, 0, RegT5, RegA0, RegT4);// add  t5, a0, t4
                    p += 4;
                    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT5, RegT5, 0);// lw  t5, 0(t5)
                    p += 4;
                ...
                {
                    *(DWORD*)p = ITypeInstr(0x3, 0x2, RegT5, RegA0, (UINT32)pLookup->sizeOffset);// lw  t5, #(pLookup->sizeOffset)(a0)
                    p += 4;
                }
            ...
            else
            {
                ...
                *(DWORD*)p = ITypeInstr(0x3, 0x2, RegA0, RegA0, (UINT32)pLookup->offsets[i]);// lw  a0, #(pLookup->offsets[i])(a0)
                p += 4;
            }
        }
