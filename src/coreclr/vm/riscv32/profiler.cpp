    ...
    : m_argIterator(pSig), m_bufferPos(0)
{
    ...
    ctx.Sp = (DWORD)pData->probeSp;
    ctx.Fp = (DWORD)pData->Fp;
    ctx.Pc = (DWORD)pData->Pc;
...
LPVOID ProfileArgIterator::CopyStructFromRegisters(const ArgLocDesc* sir)
{
    ...
    // FIXME: types
    const float *fRegBegin = &pData->floatArgumentRegisters.f[sir->m_idxFloatReg], *fReg = fRegBegin;
    const float *fRegEnd = &pData->floatArgumentRegisters.f[0] + NUM_FLOAT_ARGUMENT_REGISTERS;
    const INT32 *aRegBegin = &pData->argumentRegisters.a[sir->m_idxGenReg], *aReg = aRegBegin;
    const INT32 *aRegEnd = &pData->argumentRegisters.a[0] + NUM_ARGUMENT_REGISTERS;
    ...
    for (int i = 0; i < fieldCount; ++i)
    {
        ...
        if (fields[i].is8)
        {
            UINT32 alignedTo8 = ALIGN_UP(m_bufferPos, 8);
            _ASSERTE(alignedTo8 + 8 <= sizeof(pData->buffer));
            m_bufferPos = alignedTo8;
            const INT32* src =
                inFloatReg ? (const INT32*)fReg++ :
                inGenReg   ? aReg++ : (const INT32*)Func::postIncrement(stack, 8);
