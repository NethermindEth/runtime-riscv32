...
//
// Emulate hardware single-step on RISCV32.
//
...
//
// RiscV32SingleStepper methods.
//
RiscV32SingleStepper::RiscV32SingleStepper()
...
void RiscV32SingleStepper::Init()
{
#if !defined(DACCESS_COMPILE)
    if (m_rgCode == NULL)
    {
...
void RiscV32SingleStepper::Enable()
{
    ...
    LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Enable\n"));
...
void RiscV32SingleStepper::Bypass(uint64_t ip, uint32_t opcode)
{
    _ASSERTE(m_state != Applied);
    ...
    LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Bypass(pc=%lx, opcode=%x)\n", ip, opcode));
...
void RiscV32SingleStepper::Apply(T_CONTEXT *pCtx)
{
    if (m_rgCode == NULL)
    {
        Init();
    ...
    LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Apply(pc=%lx, opcode=%x)\n", (uint64_t)pCtx->Pc, opcode));
    ...
    {
        LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper: Case 1: Emulate\n"));
        // Case 1: Emulate an instruction that reads or writes the PC.
        m_fEmulate = true;
    }
    else
    {
        LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper: Case 2: CopyInstruction.\n"));
...
void RiscV32SingleStepper::Disable()
{
    _ASSERTE(m_state != Applied);
    m_state = Disabled;
}
...
bool RiscV32SingleStepper::Fixup(T_CONTEXT *pCtx, DWORD dwExceptionCode)
{
#ifdef _DEBUG
    uint32_t codestart = (uint32_t)m_rgCode;
    uint32_t codeend = codestart + (kMaxCodeBuffer * sizeof(uint32_t));
#endif
    ...
    if (dwExceptionCode == EXCEPTION_BREAKPOINT)
    {
        ...
        if (!m_fEmulate)
        {
            ...
            {
                LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Fixup executed code, ip = %lx\n", m_targetPc));
            ...
            else
            {
                ...
                LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Fixup emulated breakpoint\n"));
        ...
        else
        {
            ...
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Fixup emulated, ip = %lx\n", pCtx->Pc));
        }
    }
    else
    {
        ...
        LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::Fixup hit exception pc = %lx ex = %x\n", pCtx->Pc, dwExceptionCode));
    }
...
uint32_t RiscV32SingleStepper::GetReg(T_CONTEXT *pCtx, uint32_t reg)
{
    _ASSERTE(reg <= 31);
    _ASSERTE(pCtx->R0 == 0);
...
void RiscV32SingleStepper::SetReg(T_CONTEXT *pCtx, uint32_t reg, uint32_t value)
{
    _ASSERTE(reg <= 31);
...
// returned instead.
bool RiscV32SingleStepper::TryEmulate(T_CONTEXT *pCtx, uint32_t opcode, bool execute)
{
    LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate(opcode=%x, execute=%s)\n", opcode, execute ? "true" : "false"));
    ...
    if ((opcode & 0x7f) == 0x17) // AUIPC
    {
        ...
        if (execute)
        {
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate AUIPC\n"));
    ...
    else if ((opcode & 0x7f) == 0x6f) // JAL
    {
        ...
        if (execute)
        {
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate JAL\n"));
    ...
    else if ((opcode & 0x707f) == 0x67) // JALR
    {
        ...
        if (execute)
        {
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate JALR\n"));
             ...
             ((opcode & 0x707f) == 0x5063))   // BGE
    {
        ...
        if (execute)
        {
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate BEQ/BNE/BLT/BGE\n"));
             ...
             ((opcode & 0x707f) == 0x7063))   // BGEU
    {
        ...
        if (execute)
        {
            LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate BLTU/BGEU\n"));
    ...
    LOG((LF_CORDB, LL_INFO100000, "RiscV32SingleStepper::TryEmulate(opcode=%x) emulated=%s redirectedPc=%s\n",
