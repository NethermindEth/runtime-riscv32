...
//
#ifndef _VIRTUAL_CALL_STUB_RISCV32_H
#define _VIRTUAL_CALL_STUB_RISCV32_H

#define DISPATCH_STUB_FIRST_DWORD 0x00000e97
#define RESOLVE_STUB_FIRST_DWORD 0x00052e03
#define VTABLECALL_STUB_FIRST_DWORD 0x00052e83
...
public:
    ...
    void  Initialize(LookupHolder* pLookupHolderRX, PCODE resolveWorkerTarget, size_t dispatchToken)
    {
        ...
        _stub._entryPoint[1] = 0x00ceb383; //lw   t2, 12(t6)
        _stub._entryPoint[2] = 0x008ebf83; //lw   t6, 8(t6)
...
struct DispatchHolder
{
    ...
    {
        // auipc t4,0
        // lw    t0, 0(a0)     // methodTable from object in $a0
        // lw    t6, 16(t4)    // t6 _expectedMT
        // bne   t6, t0, failLabel
        // lw    t4, 20(t4)    // t4 _implTarget
        // jalr  x0, t4, 0
        // failLabel:
        // lw    t4, 24(t4)    // t4 _failTarget
        // jalr  x0, t4, 0
        //
        //
        // _expectedMT
        ...
        _stub._entryPoint[1] = 0x00052283; // lw    t0, 0(a0)     // methodTable from object in $a0
        _stub._entryPoint[2] = 0x010eaf83; // lw    t6, 16(t4)    // t6 _expectedMT
        _stub._entryPoint[3] = 0x005f9663; // bne   t6, t0, failLabel
        _stub._entryPoint[4] = 0x014eae83; // lw    t4, 20(t4)    // t4 _implTarget
        _stub._entryPoint[5] = 0x000e8067; // jalr  x0, t4, 0
        _stub._entryPoint[6] = 0x018eae83; // lw    t4, 24(t4)    // t4 _failTarget
 ...
 ********/
        ...
        // 	lw  t6, 0(t0)    # t6 = this._cacheAddress
        _stub._resolveEntryPoint[n++] = 0x0002af83 | (cacheAddressOffset << 20);
        static_assert_no_msg(((entryPointsLen+1+2) << 2) - 4 == cacheAddressOffset);
        //  add t1, t6, t1
        _stub._resolveEntryPoint[n++] = 0x006f8333;
        // 	lw  t1, 0(t1)    # t1 = e = this._cacheAddress[i]
        _stub._resolveEntryPoint[n++] = 0x00033303;

        // 	lw  t6, 0(t1)    # t6 = Check mt == e.pMT;
        _stub._resolveEntryPoint[n++] = 0x00032f83 | ((offsetof(ResolveCacheElem, pMT) & 0xfff) << 20);
        constexpr size_t tokenOffset = offsetof(ResolveStub, _token);
        // 	lw  t2, 0(t0)  #  $t2 = this._token
        _stub._resolveEntryPoint[n++] = 0x0002a383 | (tokenOffset << 20);
        static_assert_no_msg(((entryPointsLen+1+4) << 2) - 8 == tokenOffset);

        // 	bne  t6, t3, next
        _stub._resolveEntryPoint[n++] = 0x01cf9a63;// | PC_REL_OFFSET(_slowEntryPoint[0], n);

        // 	lw  t6, 0(t1)    # t6 = e.token;
        _stub._resolveEntryPoint[n++] = 0x00032f83 | ((offsetof(ResolveCacheElem, token) & 0xfff)<<20);
        // 	bne	 t6, t2, next
        ...
        // 	lw  t3, 0(t1)    # t3 = e.target;
        _stub._resolveEntryPoint[n++] = 0x00032e03 | ((offsetof(ResolveCacheElem, target) & 0xfff)<<20);
        // 	jalr x0, t3, 0
...
//#define PC_REL_OFFSET(_member, _index) (((INT32)(offsetof(ResolveStub, _member) - (offsetof(ResolveStub, _slowEntryPoint[_index])))) & 0xffff)
        ...
        // 	lw  t6, 0(t0)    # t6 = _resolveWorkerTarget;
        static_assert_no_msg((0x14*4) - 12 == ((INT32)(offsetof(ResolveStub, _resolveWorkerTarget) - (offsetof(ResolveStub, _slowEntryPoint[0])))));
        static_assert_no_msg((ResolveStub::slowEntryPointLen + ResolveStub::failEntryPointLen+1+3*2) == 0x14);
        _stub._slowEntryPoint[1] = 0x0002af83 | ((0x14 * 4) << 20);

        // 	lw  t2, 0(t0)    # t2 = this._token;
        _stub._slowEntryPoint[2] = 0x0002a383 | ((0x12 * 4) << 20); //(18*4=72=0x48)<<20
        static_assert_no_msg((ResolveStub::slowEntryPointLen+ResolveStub::failEntryPointLen+1+4)*4 == (0x12 * 4));
        static_assert_no_msg((0x12 * 4) - 8 == (offsetof(ResolveStub, _token) -offsetof(ResolveStub, _slowEntryPoint[0])));
...
//#define PC_REL_OFFSET(_member, _index) (((INT32)(offsetof(ResolveStub, _member) - (offsetof(ResolveStub, _failEntryPoint[_index])))) & 0xffff)
        ...
        // 	lw  t1, 0(t0)    # t1 = _pCounter;  0x2800000=((failEntryPointLen+1)*4)<<20.
        _stub._failEntryPoint[1] = 0x0002a303 | 0x2800000;
...
struct VTableCallStub
{
    ...
    inline size_t size()
    {
        ...
        // lw t4, 0(t4)
        if ((*(DWORD*)(&pStubCode[16])) == 0x000eae83)
        {
            if ((*(DWORD*)(&pStubCode[28])) == 0x000eae83)
                cbSize += 12;
        }
...
void VTableCallHolder::Initialize(unsigned slot)
{
    ...
    // lw  t4, 0(a0) : t4 = MethodTable pointer
    *(UINT32*)p = 0x00053e83; // VTABLECALL_STUB_FIRST_DWORD
    ...
    if (offsetOfIndirection > 2047)
    {
        ...
        *(DWORD*)p = 0x01ce8eb3; p += 4;
        // lw t4, 0(t4)
        *(DWORD*)p = 0x000eae83; p += 4;
    }
    else
    {
        // lw t4, offsetOfIndirection(t4)
        *(DWORD*)p = 0x000eae83 | ((UINT32)offsetOfIndirection << 20); p += 4;
    }
    ...
    {
        // lw t4, offsetAfterIndirection(t4)
        *(DWORD*)p = 0x000eae83 | ((UINT32)offsetAfterIndirection << 20); p += 4;
    }
...
#endif // _VIRTUAL_CALL_STUB_RISCV32_H
