...
struct MachState {
    ULONG          captureCalleeSavedRegisters[NUM_NONVOLATILE_CONTEXT_POINTERS]; // preserved registers
    PTR_ULONG      ptrCalleeSavedRegisters[NUM_NONVOLATILE_CONTEXT_POINTERS]; // pointers to preserved registers
...
inline void LazyMachState::setLazyStateFromUnwind(MachState* copy)
{
    ...
    PULONG32* pSrc = (PULONG32 *)&copy->ptrCalleeSavedRegisters;
    PULONG32* pDst = (PULONG32 *)&this->ptrCalleeSavedRegisters;

    const PULONG32 LowerBoundDst = (PULONG32) this;
    const PULONG32 LowerBoundSrc = (PULONG32) copy;

    const PULONG32 UpperBoundSrc = (PULONG32) ((BYTE*)LowerBoundSrc + sizeof(*copy));

    for (int i = 0; i < NUM_NONVOLATILE_CONTEXT_POINTERS; i++)
    {
        PULONG32 valueSrc = *pSrc++;

        if ((LowerBoundSrc <= valueSrc) && (valueSrc < UpperBoundSrc))
        {
            // make any pointer interior to 'src' interior to 'dst'
            valueSrc = (PULONG32)((BYTE*)valueSrc - (BYTE*)LowerBoundSrc + (BYTE*)LowerBoundDst);
        }
