...
#error Should only include "cGenCpu.h" for RISCV32 builds
#endif
...
#define ENREGISTERED_RETURNTYPE_INTEGER_MAXSIZE 8   // bytes (two int registers: a0 and a1)
#define ENREGISTERED_PARAMTYPE_MAXSIZE          8   // bytes (max value type size that can be passed by value)

#define CALLDESCR_ARGREGS                       1   // CallDescrWorker has ArgumentRegister parameter
#define CALLDESCR_FPARGREGS                     1   // CallDescrWorker has FloatArgumentRegisters parameter

#define FLOAT_REGISTER_SIZE 4 // each register in FloatArgumentRegisters is 8 bytes.
...
struct CalleeSavedRegisters {
    ...
    INT32 ra; // return register
    INT32 s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11;
    INT32 tp, gp;
};
...
struct ArgumentRegisters {
    INT32 a[NUM_ARGUMENT_REGISTERS]; // a0 ....a7
};
...
struct FloatArgumentRegisters {
    //TODO: not supports RISCV32-SIMD.
    float  f[NUM_FLOAT_ARGUMENT_REGISTERS];  // f0-f7
};
...
inline void emitJump(LPBYTE pBufferRX, LPBYTE pBufferRW, LPVOID target)
{
    ...
    *((LPVOID *)(pCode + 4)) = target;   // 32-bit target address
}
...
public:
    ...
    void EmitMovConstant(IntReg target, UINT32 constant);
...
struct HijackArgs
{
    DWORD Fp; // frame pointer
    union
    {
        DWORD Ra;
        size_t ReturnAddress;
    };
    DWORD S1, S2, S3, S4, S5, S6, S7, S8, S9, S10, S11, Gp, Tp;
    union
    {
        struct {
             DWORD A0;
