...
#ifndef __RISCV32_SINGLE_STEPPER_H__
#define __RISCV32_SINGLE_STEPPER_H__

// Class that encapsulates the context needed to single step one thread.
class RiscV32SingleStepper
{
public:
    RiscV32SingleStepper();
    ~RiscV32SingleStepper();
...
private:
    enum
    {
        ...
        kBreakpointOp = 0x00100073, // Opcode for the breakpoint instruction used on RiscV32 Unix
    };
    ...
    uint32_t      m_originalPc;               // PC value before stepping
    uint32_t      m_targetPc;                 // Final PC value after stepping if no exception is raised
    ...
    uint32_t GetReg(T_CONTEXT *pCtx, uint32_t reg);
    // Set the current value of a register.
    void SetReg(T_CONTEXT *pCtx, uint32_t reg, uint32_t value);
...
#endif // !__RISCV32_SINGLE_STEPPER_H__
